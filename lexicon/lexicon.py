...
DU_CALENDAR: dict[str, str | dict] = {
    ...
    'calendar': 'Выбери дату когда ты хочешь <b>организовать</b> либо <b>посетить мероприятие</b>. На датах с <b>восклицательным знаком (❗)</b> есть мероприятия:',
    'choice': 'Вот что ты можешь сделать на дату <b>{{date}}</b> в <b>{{city}}</>:{% if full_events %}\n\n❗ Максимально можно создать не больше<b> 5 мероприятий</>! Дождись их окончания, либо удали несколько в <b>личном кабинете</b>⚠️{% endif %}',
...
DU_JOIN_EVENTS: dict[str, str | dict] = {
    'result': "{% if has_events %}{{description}}\n\n────────────────────\n\n🎯 <b>Событие: </b>{{title}}\n\n🏙 <b>Локация: </b>{{city}}\n\n📆 <b>Дата проведения: </b>{{date}}\n\n👥 Количество участников: {{participants}}\n\n💬 <b>Для связи: </b>@{{username}}{% else %} У тебя пока что нет запланированных мероприятий. Перейди в /calendar чтобы запланировать мероприятие {% endif %}",
    'buttons': {
        'cancel_event': '❌ Я НЕ пойду!',
        'next_event': ">>",
        'prev_event': "<<"
...
DU_MY_EVENTS: dict[str, str | dict] = {
    'result': "{% if has_events %}{{description}}\n\n────────────────────\n\n🎯 <b>Событие: </b>{{title}}\n\n🏙 <b>Локация: </b>{{city}}\n\n📆 <b>Дата проведения: </b>{{date}}\n\n👥 Количество участников: {{participants}}\n\n💬 <b>Для связи: </b>@{{username}}{% else %} У тебя пока что нет созданных мероприятий. Перейди в /calendar чтобы создать мероприятие {% endif %}",
    'buttons': {
